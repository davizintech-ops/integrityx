...
item "「INFO」Crawling Android Developers for latest Pixel Beta device list ...";
...
item "「INFO」Selecting Pixel Beta device ...";
if [ -z "$PRODUCT" ]; then
  set_random_beta() {
...
item "「INFO」Crawling Android Flash Tool for latest Pixel Canary build info ...";
...
[ -z "$ID" -o -z "$INCREMENTAL" ] && die "「ERR」Failed to extract build info from JSON";
...
if [ -z "$SECURITY_PATCH" ]; then
  warn " 「WARN」Failed to determine exact security patch level from Pixel Update Bulletins";
  item "Assuming probable security patch level from Canary build info ...";
  SECURITY_PATCH="${CANARY_ID}-05";
fi;
echo "$SECURITY_PATCH";

item "「INFO」Dumping values to minimal pif.prop ...";
...
if [ -f "$MIGRATE" ]; then
  ...
  if [ "$FORCE_STRONG" ]; then
    item "「INFO」Forcing configuration for <A13 PI Strong ...";
    ARGS="-a"; PATCH_COMMENT=1; spoofProvider=0;
  else
    item "「INFO」Retaining existing configuration ...";
  fi;
...
EOF
      ...
      item "「INFO」Updating Tricky Store security_patch.txt ...";
  ...
  if [ -f /data/adb/modules/playintegrityfix/killpi.sh ]; then
    item "「INFO」Killing any running GMS DroidGuard/Play Store processes ...";
